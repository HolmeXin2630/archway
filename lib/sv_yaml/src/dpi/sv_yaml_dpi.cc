...
int dpi_yaml_parse_file(const char* input) {
    if (!input) { SET_ERROR("null input"); return 0; }
    std::ifstream f(input);
    if (!f.is_open()) {
        SET_ERROR("YAML parse_file: cannot open file '%s'", input);
        return 0;
    }
    std::string content((std::istreambuf_iterator<char>(f)),
                         std::istreambuf_iterator<char>());
    // Try as YAML content (non-scalar)
    int result = try_parse_yaml_content(content);
    if (result > 0) return result;
    // Fallback: scalar
    try {
        ...
        SET_ERROR("YAML file parse error: %s -- %s", input, e.what());
    } catch (...) {
        SET_ERROR("YAML file parse error: %s", input);
    }
    return 0;
}
